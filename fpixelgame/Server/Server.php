...
namespace fpixelgame\Server;

use fpixelgame\FPixelGame;
use fpixelgame\Worker;
use fpixelgame\ServerLoad;
...
class Server{
	...
	public function about(){
	 $version = implode(",",ProtocolInfo::MINECRAFT_VERSION);
		$string = "
FillAmeaPixel FPixelGame - Core v2.0  优化核心
基于FillAmeaPixel - Core GenisysPro
更多支持库  自带多种API
经济 多世界  红石 bossbar 等等
BUG反馈:1602401265@qq.com
Github: FillAmeaPixelNetWork
...
Github:FillAmeaPixelNetWork
  ...
  if($this->dserverConfig["enable"] and $this->dserverConfig["motdPlayers"]){
			 ...
			 $name = $this->getNetwork()->getName().'['.爆满.'/'.爆满.']';
			 $this->getNetwork()->setName($name);
			 //FillAmeaPixelNetWork
			}
